...
async function connectRedis(redisUrl, cmd) {
  ...
  try {
    ...
    global.redis = new Proxy(redis, {
      get(target, p, receiver) {
        const ret = Reflect.get(target, p, receiver);
        if (typeof ret === 'function') {
          return new Proxy(ret, {
            apply: (target2, thisArg, args) => {
              const ret = Reflect.apply(target2, thisArg, args);
              Bot.makeLog("trace", `redis.${p} with ${args} = ${ret}`)
              return ret;
            }
          });
        }
        Bot.makeLog("trace", `redis:${typeof p === 'string' ? p : 'symbol'}`)
        return ret;
      }
    })
    await redis.connect()
