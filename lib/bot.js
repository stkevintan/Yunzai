...
export default class Yunzai extends EventEmitter {
  ...
  wsConnect(req, socket, head) {
    this.wss.handleUpgrade(req, socket, head, conn => {
      ...
      conn.on("close", (code, reason) => this.makeLog("mark", `断开连接 ${code} ${reason}`, `${conn.sid} <≠> ${conn.rid}`))
      ...
      this.makeLog("info", `启动ws ${req.url} ${req.url.split("/")[1]}  ${Object.keys(this.wsf).join()} ${this.wsf[req.url.split("/")[1]]}`);
  ...
  async sendMasterMsg(msg, bot_array = Object.keys(cfg.master)) {
    const ret = []
